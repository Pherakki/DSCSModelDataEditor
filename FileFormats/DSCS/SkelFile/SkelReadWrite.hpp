...
namespace FileFormats::DSCS::SkelFile
{
    ...
    private:
        ...
        uint32_t          rel_ptr_to_uv_data_2 = 0;
        uint32_t          rel_ptr_to_uv_data_3 = 0;
        ...
        uint32_t getAbsPtrToUVData2() { return rel_ptr_to_uv_data_2 + 0x24; };
        uint32_t getAbsPtrToUVData3() { return rel_ptr_to_uv_data_3 + 0x28; };
        uint32_t getAbsPtrToUVData1() { return rel_ptr_to_uv_data_1 + 0x2C; };

        std::vector<std::array<uint16_t, 8>> bone_heirarchy = {};
        std::vector<std::array<float, 12>>   rest_pose_data = {};
        std::vector<uint16_t>                bone_parents = {};
        std::vector<uint8_t>                 uv_data_1 = {};
        std::vector<uint32_t>                bone_name_hashes = {};
        std::vector<uint32_t>                uv_data_2 = {};
        std::vector<uint32_t>                uv_data_3 = {};
        ...
        template <std::ios_base::openmode openmode_flag> void readWriteUVData2(ReadWriter<openmode_flag>&);
        template <std::ios_base::openmode openmode_flag> void readWriteUVData3(ReadWriter<openmode_flag>&);
    ...
    void SkelReadWrite::readWriteHeader(ReadWriter<openmode_flag>& read_writer)
    {
        ...
        read_writer.ReadWriteData<uint16_t, LE>(this->num_uv_sliders);
        ...
        read_writer.ReadWriteData<uint32_t, LE>(this->rel_ptr_to_uv_data_2);
        read_writer.ReadWriteData<uint32_t, LE>(this->rel_ptr_to_uv_data_3);
    ...
    {
        this->bone_heirarchy =   std::vector<std::array<uint16_t, 8>>(num_bone_hierarchy_data_rows);
        this->rest_pose_data =   std::vector<std::array<float, 12>>(num_bones);
        this->bone_parents =     std::vector<uint16_t>(num_bones);
        this->uv_data_1 =        std::vector<uint8_t>(num_uv_sliders);
        this->bone_name_hashes = std::vector<uint32_t>(num_bones);
        this->uv_data_2 =        std::vector<uint32_t>(num_uv_sliders);
        this->uv_data_3 =        std::vector<uint32_t>(num_uv_sliders);
    }
    template<>
    ...
    {
        read_writer.ReadWriteDataArray<std::array<uint16_t, 8>, LE>(this->bone_heirarchy);
    }
    ...
    void SkelReadWrite::readWriteRestPoseData(ReadWriter<openmode_flag>& read_writer)
    {
        ...
        read_writer.ReadWriteDataArray<std::array<float, 12>, LE>(this->rest_pose_data);
    }
    ...
    void SkelReadWrite::readWriteParentBones(ReadWriter<openmode_flag>& read_writer)
    {
        ...
        read_writer.ReadWriteDataArray<uint16_t, LE>(this->bone_parents);
    }
    ...
    void SkelReadWrite::readWriteUVData1(ReadWriter<openmode_flag>& read_writer)
    {
        ...
        read_writer.ReadWriteDataArray<uint8_t, LE >(this->uv_data_1);
    }
    ...
    void SkelReadWrite::readWriteBoneNameHashes(ReadWriter<openmode_flag>& read_writer)
    {
        ...
        read_writer.ReadWriteDataArray<uint32_t, LE >(this->bone_name_hashes);
    }

    // readWriteUVData2
    template <std::ios_base::openmode openmode_flag>
    void SkelReadWrite::readWriteUVData2(ReadWriter<openmode_flag>& read_writer)
    {
        read_writer.AssertFilePointerIsAt(this->getAbsPtrToUVData2());
        read_writer.ReadWriteDataArray<uint32_t, LE >(this->uv_data_2);
    }

    // readWriteUVData3
    template <std::ios_base::openmode openmode_flag>
    void SkelReadWrite::readWriteUVData3(ReadWriter<openmode_flag>& read_writer)
    {
        read_writer.AssertFilePointerIsAt(this->getAbsPtrToUVData3());
        read_writer.ReadWriteDataArray<uint32_t, LE >(this->uv_data_3);
    }
    ...
    void SkelReadWrite::doReadWrite(const std::string& filepath)
    {
        ...
        read_writer.ReadWriteBytesLeftInIncompleteChunk(pos, 16);
        this->readWriteBoneNameHashes(read_writer);
        this->readWriteUVData2(read_writer);
        this->readWriteUVData3(read_writer);
        pos = read_writer.Tell();
        read_writer.ReadWriteBytesLeftInIncompleteChunk(pos - this->num_bytes_in_name_hashes_section, 16);
